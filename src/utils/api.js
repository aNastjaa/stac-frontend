...
export const login = async (email, password, rememberMe, csrfToken // Expecting CSRF token from the frontend
) => {
...
export const getUserProfileByProfileId = async (profileId) => {
    try {
        const csrfToken = getCsrfTokenFromCookie();
        const decodedCsrfToken = decodeURIComponent(csrfToken);
...
export const getProfileIdByUserId = async (userId) => {
    try {
        ...
        const result = await response.json();
        const profile = result.profiles.find((profile) => profile.user_id === userId);
...
//Fetch avatar URL
export const fetchAvatarUrl = async (avatarId) => {
    try {
