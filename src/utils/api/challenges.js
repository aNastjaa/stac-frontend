...
export const submitWork = async (challengeId, formData, csrfToken) => {
    try {
...
export const deleteSubmission = async (submissionId, challengeId) => {
    try {
...
export const voteForSubmission = async (challengeId, submissionId) => {
    try {
        const csrfToken = getCsrfTokenFromCookie();
        const authToken = localStorage.getItem("auth_token");
        if (!authToken) {
...
export const removeVote = async (challengeId, submissionId) => {
    try {
        const csrfToken = getCsrfTokenFromCookie();
        const authToken = localStorage.getItem("auth_token");
        if (!authToken) {
...
export const checkUserVote = async (challengeId, submissionId) => {
    try {
        const csrfToken = getCsrfTokenFromCookie();
        const authToken = localStorage.getItem("auth_token");
        if (!authToken) {
        ...
        const data = await response.json();
        return data.some((vote) => vote.user_id === getUserIdFromLocalStorage());
    }
    catch (error) {
